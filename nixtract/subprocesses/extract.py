...
def extract(
    ...
    finder_process.wait()
    logger.info("Finder process is done")
    reader_thread.join()
    logger.info("Reader process is done")
    process_queue_thread.join()
    logger.info("Queue processor process is done")
    derivation_description_pool.close()
    derivation_description_pool.join()
    logger.info("Derivation description pool is done")

    if not derivation_description_queue.empty():
        raise Exception("All done but queue is not empty! This should never happen.")
